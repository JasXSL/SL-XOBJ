...
You should have a #ROOT script and the cl Dialog module installed (see the SL-XOBJ original readme).

Add a new script to your project and name it "st Main"
Navigate locally to your "myFirstProject" or whatever you named your project folder.
Create a new subfolder and name it "classes", so you'll have something like "c:\LSL\myFirstProject\classes"
Inside that folder, create a new file and name it "st Main.lsl"
Open the file in a text editor of choice (I recommend notepad++)
Note: Make sure the notecard ends with a blank row, or the preprocessor will throw a fit.
Save the blank script for now, we'll use it later.
Open up your "st Main" script in SL.
On the first row enter #include "myFirstProject/_core.lsl"
On the second row enter #include "myFirstProject/classes/st Main.lsl"
On the third row enter #include "xobj_core/classes/cl Dialog.lsl"

Note that your script is by default defined as a static script, whereas if you wanted the script ot be a package of objects you'd also include xobj_core/_CLASS_PACKAGE.lsl - But that's very advanded so we'll just start with static modules.

Next we'll need to start listening to events. After the #includes add the following code:
onEvt(string script, integer evt, string data){
	if(script == "#ROOT" && evt == evt$TOUCH_START){
...
Now you're listening to project events. In particular you are checking if the sender script was named "#ROOT" and the event was the standard event evt$TOUCH_START.
...
Let's take a look at how you can let other modules access yours. Start off by opening your st Main.lsl file.
On the first line add #define MainMethod$helloWorld 1	// (key)sender - Sends a hello world message with sender's display name
You have defined your first method identifier (1)! And it accepts 1 argument (key)sender
Save the file and go back to your st Main SL script.

After the if(method$isCallback) if statement (right above #define LM BOTTOM) add the following code:
...
5. Add to the top of the script: #include "myFirstProject/classes/st Main.lsl"
...
if(method$isCallback){
    if(SENDER_SCRIPT == "st Main" && METHOD == MainMethod$helloWorld){
...
touch_start(integer total){
	runOmniMethod("st Main", MainMethod$hellOWorld, [llDetectedKey(0)], "", "", "This is a callback message", "");
}
