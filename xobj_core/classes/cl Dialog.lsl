...
#define Dialog$spawn(user, message, buttons, menuID, callback) runMethod((string)LINK_THIS, "cl Dialog", DialogMethod$spawn, [user, message, llList2Json(JSON_ARRAY, buttons), menuID], TARG_NULL, callback, "")
// Checks in LM callback section if a callback method is a dialog callback, ex if(Dialog$isCallback){}
#define Dialog$isCallback (SENDER_SCRIPT == "cl Dialog" && METHOD == DialogMethod$spawn)
